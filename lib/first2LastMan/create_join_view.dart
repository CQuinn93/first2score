...
import 'package:application/first2Score/join_competition_view.dart';
import 'package:application/first2Score/make_selections_screen.dart';
...
class CreateJoinViewState extends State<CreateJoinView> {
  ...
  String? _joinCode;
  int? _selectedLives; // Track number of lives
  int _currentGameweek = 1; // Assume this will be fetched from an API or Supabase

  @override
  void initState() {
    super.initState();
    _fetchCurrentGameweek(); // Fetch the current gameweek on load
  }

  // Fetch the current gameweek
  Future<void> _fetchCurrentGameweek() async {
    try {
      // Fetch current gameweek from your backend or API
      ...
      setState(() {
        _currentGameweek = response['current_gameweek'] ?? 1; // Update state with current gameweek
      });
    } catch (error) {
      if (kDebugMode) print("Error fetching current gameweek: $error");
    }
  }
  ...
  void _createCompetition() async {
    ...
    try {
      ...
      final List<Map<String, dynamic>> competitionResponse =
        ...
        'competition_detail': "First2LastMan",
        'is_complete': false,
        'lives': _selectedLives, // Add the selected lives for each player
      }).select();
        ...
        children: [
          ...
          Expanded(
            child: IndexedStack(
              ...
              children: [
                _joinView(), // Create competition view
                _createView(), // Join competition view
              ],
            ),
          ),
        ],
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            // Game Week Dropdown (only future gameweeks)
            InputDecorator(
              decoration: InputDecoration(
                labelText: 'Game Week',
              ...
              child: DropdownButtonHideUnderline(
                child: DropdownButton<int>(
                  ...
                  style: TextStyle(color: themeTextColour),
                  items: List.generate(36 - _currentGameweek, (index) {
                    return DropdownMenuItem(
                      value: _currentGameweek + index + 1,
                      child: Text('Game Week ${_currentGameweek + index + 1}'),
                    );
                  }),
                  onChanged: (value) {
                    setState(() {
            ...
            InputDecorator(
              ...
              child: DropdownButtonHideUnderline(
                child: DropdownButton<int>(
                  ...
                  onChanged: (value) {
                    setState(() {
                      _selectedLives = value;
                    });
                  },
                  hint: Text('Select Number of Lives',
                      style: TextStyle(color: themeHintTextColour)),
                ),
              ),
            ),
